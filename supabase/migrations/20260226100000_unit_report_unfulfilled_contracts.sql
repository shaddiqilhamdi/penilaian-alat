-- Add Section 5: Equipment Kontrak Belum Terpenuhi to fn_unit_report
-- Shows equipment where kesesuaian_kontrak < 2 (contract not fulfilled)
-- Joins assessment_items to get required_qty for display
...
BEGIN
    ...
    -- 2. Breakdown per Peruntukan + Regu count (DISTINCT teams)
    ...
    FROM (
        SELECT jsonb_build_object(
            ...
            'total_equipment', COUNT(*),
            'total_regu', COUNT(DISTINCT va.team_id) FILTER (WHERE va.owner_id = va.team_id AND va.team_id IS NOT NULL),
            'avg_score', ROUND(COALESCE(AVG(va.nilai), 0)::NUMERIC, 2),
            'tl_fisik', COUNT(*) FILTER (WHERE va.kondisi_fisik = -1),
            'tb_fungsi', COUNT(*) FILTER (WHERE va.kondisi_fungsi = -1)
        ) AS row_data,
    ...
    -- 4. Equipment Bermasalah + owner_label
    ...
    -- 5. Equipment Kontrak Belum Terpenuhi
    -- Shows items where kesesuaian_kontrak < 2
    -- Joins assessment_items to get required_qty for comparison
    SELECT COALESCE(jsonb_agg(row_data ORDER BY peruntukan_sort, nama_sort), '[]'::jsonb)
    INTO v_unfulfilled_contracts
    FROM (
        SELECT jsonb_build_object(
            'id', va.id,
            'nama_alat', em.nama_alat,
            ...
            'peruntukan', p.deskripsi,
            'jenis', em.jenis,
            'required_qty', COALESCE(ai.required_qty, 0),
            'realisasi_qty', COALESCE(va.realisasi_qty, 0),
            'selisih', COALESCE(ai.required_qty, 0) - COALESCE(va.realisasi_qty, 0),
            'owner_label', CASE
                WHEN va.owner_id = va.team_id AND va.team_id IS NOT NULL 
                    THEN COALESCE(t.nomor_polisi, t.id::TEXT)
                ELSE COALESCE(per.nama_personil, per.nik, '-')
        ...
        COALESCE(p.deskripsi, 'Lainnya') AS peruntukan_sort,
        em.nama_alat AS nama_sort
        FROM vendor_assets va
        INNER JOIN vendors v ON va.vendor_id = v.id
        LEFT JOIN peruntukan p ON va.peruntukan_id = p.id
        LEFT JOIN equipment_master em ON va.equipment_id = em.id
    ...
    v_result := jsonb_build_object(
        ...
        'unfulfilled_contracts', v_unfulfilled_contracts,
